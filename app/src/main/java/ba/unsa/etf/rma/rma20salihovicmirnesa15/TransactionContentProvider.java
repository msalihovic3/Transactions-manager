...
public class TransactionContentProvider extends ContentProvider {
    ...
    static {
        ...
        uM.addURI("rma.provider.transaction","elements",ALLROWS);
    ...
    @Override
    public int delete(@NonNull Uri uri, @Nullable String _id, @Nullable String[] strings) {
        int count = 0;
        SQLiteDatabase db;
        try{
        ...
        switch (uM.match(uri)){
            case ALLROWS:
                count = db.delete(TransactionDBOpenHelper.TRANSACTION_TABLE, _id, strings);
    ...
    public int update(@NonNull Uri uri, @Nullable ContentValues values, @Nullable String s, @Nullable String[] strings) {
        ...
        switch (uM.match(uri)){
            case ALLROWS:
                ...
                System.out.println("samo jesan"+s+strings);
                count = db.update(TransactionDBOpenHelper.TRANSACTION_TABLE, values, TransactionDBOpenHelper.TRANSACTION_ID + s, strings);

                break;

            case ONEROW:
                 System.out.println("samo jesan"+s);
                count = db.update(TransactionDBOpenHelper.TRANSACTION_TABLE, values, TransactionDBOpenHelper.TRANSACTION_ID +s , strings);
